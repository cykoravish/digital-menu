...
const Ads = () => {
  ...
  const fetchAds = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_API}/ads/manage`, {
        headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_API}/ads/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("superadmin_token")}`,
        },
    ...
    <div className="p-6 text-gray-900">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600">Manage banner ads shown to free plan users</p>
        </div>
        <button
          ...
          <div key={ad._id} className="bg-white rounded-lg shadow-md overflow-hidden">
            <div className={`h-32 bg-gradient-to-r ${ad.gradient} relative`}>
              ...
              <div className="absolute top-2 right-2 flex space-x-1">
                {ad.isActive ? <Eye className="w-4 h-4 text-white" /> : <EyeOff className="w-4 h-4 text-white/60" />}
              </div>
            </div>
            ...
            <div className="p-4">
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex space-x-2">
                  <button onClick={() => openEditModal(ad)} className="text-blue-600 hover:text-blue-800">
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold">{editingAd ? "Edit Ad" : "Create New Ad"}</h2>
                <button
                  onClick={() => {
                    setShowModal(false)
                    resetForm()
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                    {gradientOptions.map((option) => (
                      <label key={option.value} className="cursor-pointer">
                        ...
                        />
                        <div
                          className={`h-8 rounded-lg ${option.preview} border-2 ${
                            formData.gradient === option.value ? "border-blue-500" : "border-gray-200"
                          }`}
                        ></div>
