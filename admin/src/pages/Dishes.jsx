      ...
      <div>
        <div className="flex items-start justify-between mb-2">
          ...
          <div className="flex items-center space-x-1">
            <span
              className={`w-3 h-3 rounded-full ${dish.isVeg ? "bg-green-500" : "bg-red-500"}`}
              title={dish.isVeg ? "Vegetarian" : "Non-Vegetarian"}
            />
            <span
              className={`px-2 py-1 rounded-full text-xs font-medium ${
                dish.isAvailable ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"
              }`}
            >
      ...
      >
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-gray-900">{dish ? "Edit Dish" : "Add New Dish"}</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select name="spiceLevel" value={formData.spiceLevel} onChange={handleChange} className="input w-full">
...
const Dishes = () => {
  ...
  const handleSaveDish = (savedDish) => {
    if (editingDish) {
      setDishes(dishes.map((dish) => (dish._id === savedDish._id ? savedDish : dish)))
    } else {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredDishes.map((dish) => (
          <DishCard key={dish._id} dish={dish} onEdit={handleEditDish} onDelete={handleDeleteDish} />
        ))}
      </div>
      ...
      <DishModal dish={editingDish} isOpen={modalOpen} onClose={() => setModalOpen(false)} onSave={handleSaveDish} />
    </div>
  )
}
